...
class AzureIoTHub
{
    ...
    //
    // Note: this connection string is specific to the device. To configure other devices,
    // see information on iothub-explorer at http://aka.ms/iothubgetstartedVSCS
    //
    const string deviceConnectionString = "{IoT Device Connection String}";
    ...
    public static async Task SendDeviceToCloudMessageAsync()
    {
        CreateClient();
#if WINDOWS_UWP
        var str = "{\"deviceId\":\"{Device ID}\",\"messageId\":1,\"text\":\"Hello, Cloud from a UWP C# app!\"}";
#else
        var str = "{\"deviceId\":\"{Device ID}\",\"messageId\":1,\"text\":\"Hello, Cloud from a C# app!\"}";
#endif
